...
import { transactions as allTx, type Transaction, type TxType } from "@/data/transactions";
...
import { SearchInput } from "@/components/wallet/SearchInput";
import { TransactionFilters, type Filter } from "@/components/wallet/TransactionFilters";
  ...
  const filtered = useMemo(() => {
    ...
    return allTx
      .filter((t) => {
        ...
        // map our Filter type to TxType
        const ft: TxType = filter === "sent" ? "sent" : "received";
        return t.type === ft;
      })
      .filter((t) => (t.merchant + " " + t.subtitle).toLowerCase().includes(q));
  }, [query, filter]);
      ...
      <header className="px-5 pt-5 pb-4">
        ...
        <div className="mt-4 flex gap-3">
          <div className="flex-1">
            <SearchInput value={query} onChange={(v) => setQuery(v)} placeholder="Search transactions" />
          </div>

          <div className="w-30 mt-2">
            <TransactionFilters value={filter} onChange={(v) => setFilter(v)} />
          </div>
        </div>
      </header>
